...
export function MoviezzForYouScreen({ navigation }) {
    const { setTrendingMoviesPage, trendingMoviesPage, trendingMovies, getTrendingMovies, handleCardRedirection } = useContext(AppContext)

    const handleSwitchPageRight = () => {
        const MAX_PAGES_TRENDINGMOVIES = 43158
        let currentNumber = trendingMoviesPage
        let nextNumber = currentNumber + 1

        if (trendingMoviesPage !== MAX_PAGES_TRENDINGMOVIES) {
            setTrendingMoviesPage(nextNumber)
        }
    }
    const handleSwitchPageleft = () => {
        const MIN_PAGES_TRENDINGMOVIES = 1
        let currentNumber = trendingMoviesPage
        let nextNumber = currentNumber - 1

        if (trendingMoviesPage !== MIN_PAGES_TRENDINGMOVIES) {
            setTrendingMoviesPage(nextNumber)
        }
    }
    useEffect(() => {
    ...
    return (
        <SafeAreaView style={styles.background}>
            ...
            <SubTitle firstWord='Moviezz for you' hideSeeAll={true}></SubTitle>
            ...
            <View style={styles.buttonSwitchContainer}>
                ...
                <Text style={styles.text}>{trendingMoviesPage}</Text>
